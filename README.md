...
| `kb_autotag` | Auto-extract tags from KB file content and write them to frontmatter. Improves `kb_ask` search accuracy when files have empty `tags: []`. Extracts from headings (3×), bold text, inline code, file path, and body word frequency. Run `file_path: "all"` (default) or target a single file. Three modes: `fast` (default, regex extracts and overwrites tags); `review` (returns scored candidates grouped as preserved/new/possibly_stale for an LLM to validate — does not write); `apply` (writes LLM-reviewed tags from a `tags: { path: [...] }` map). Tags are system-owned and replaced on each run |
...
- **`type`** — document type. Folder-inferred values from `lib/types.js::inferType()`: `feature`, `flow`, `schema`, `validation`, `integration`, `decision`, `standard`, `component`, `data`. Template-set values that `inferType` doesn't return but templates use explicitly: `group` (folder notes), `reference` (e.g. `knowledge/glossary.md`). Stored in `_index.yaml` and used for keyword-based scoring in `kb_get` / `kb_impact`.
...
- **`cssclasses`** — per-type CSS class for theming (`kb-feature`, `kb-flow`, `kb-schema`, `kb-validation`, `kb-integration`, `kb-decision`, `kb-component`, `kb-group`, `kb-reference`). Add a `.obsidian/snippets/kb.css` to activate visual differentiation in the graph view.
