...
import type { getStatus as GetStatus } from '@instrumentality/shared'
...
// under dist/. The runtime require targets the built dist explicitly so the
// error message can guide the user to build shared; the type comes from the
// package's published declarations.
function loadShared(): { getStatus: typeof GetStatus } {
  const repoRoot = pkgPaths.repoRoot()
  const sharedDist = path.join(repoRoot, 'packages', 'shared', 'dist', 'status.js')
  try {
    return require(sharedDist) as { getStatus: typeof GetStatus }
  } catch (err) {
    throw new Error(
      `kb_status requires the @instrumentality/shared package to be built. Run \`cd packages/shared && npm install && npm run build\` from the repo root. (${(err as Error).message})`
    )
  }
}
...
async function runTool(args?: Record<string, unknown>): Promise<Record<string, unknown>> {
  ...
  const skipLint = args ? args.skip_lint === true : false
  const includeEvents = args ? args.include_events === true : false
  ...
  if (includeEvents) return summary as unknown as Record<string, unknown>
  return trimDriftLog(summary as unknown as Record<string, unknown>)
}

function trimDriftLog(summary: Record<string, unknown>): Record<string, unknown> {
  if (!summary || !Array.isArray(summary.driftLogEvents)) return summary
  const { driftLogEvents, ...rest } = summary as Record<string, unknown> & { driftLogEvents: unknown[] }
  return {
    ...rest,
...
// trimDriftLog is exposed for tests; not part of the MCP surface.
export { runTool, trimDriftLog, definition }
